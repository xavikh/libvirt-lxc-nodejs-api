...
new_client = function(client, req) {
    ...
    console.log("TokenRel: " + tokenFile[queries.token]);
    if(!tokenFile[queries.token]){
        console.log("Rejected");
        return;
    }
    let tokenHostPort = tokenFile[queries.token].split(':');

    var target = net.createConnection(tokenHostPort[1], tokenHostPort[0], function() {
